...
using IsLessInputs = boost::mp11::mp_list<
    ...
    IsLessInputType<
        ctu::UnitDimension<TestUnitA, -1>, ctu::UnitDimension<TestUnitB, 1>>,
    IsLessInputType<
        ctu::UnitDimension<TestUnitA, 1>, ctu::UnitDimension<TestUnitB, -1>>>;
...
{
    BOOST_STATIC_ASSERT(
        ctu::ud_operations::IsLess<
            typename IsLessInput::input_a, typename IsLessInput::input_b>());
}
...
BOOST_AUTO_TEST_CASE_TEMPLATE(TestIsLessNegative, IsLessInput, IsLessInputs)
{
    BOOST_STATIC_ASSERT(
        !ctu::ud_operations::IsLess<
            typename IsLessInput::input_b, typename IsLessInput::input_a>());
}

BOOST_AUTO_TEST_CASE_TEMPLATE(
    TestIsLessFunctionPositive, IsLessInput, IsLessInputs)
{
    const bool result = ctu::ud_operations::is_less<
        typename IsLessInput::input_a, typename IsLessInput::input_b>();
    BOOST_CHECK(result);
}

BOOST_AUTO_TEST_CASE_TEMPLATE(
    TestIsLessFunctionNegative, IsLessInput, IsLessInputs)
{
    const bool result = !ctu::ud_operations::is_less<
        typename IsLessInput::input_b, typename IsLessInput::input_a>();
    BOOST_CHECK(result);
